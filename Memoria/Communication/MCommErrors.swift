...
class MCommErrors: NSObject {
    ...
    func getError(error: AFError?, httResponse: HTTPURLResponse?) -> (errorCode: Int, description: String?) {
        ...
        if let error = error {
            switch error {
            case let .createUploadableFailed(error as NSError):
                return (error.code, error.localizedDescription)
            case let .createURLRequestFailed(error as NSError):
                return (error.code, error.localizedDescription)
            case let .requestAdaptationFailed(error as NSError):
                return (error.code, error.localizedDescription)
            case let .sessionInvalidated(error as NSError):
                return (error.code, error.localizedDescription)
            case let .sessionTaskFailed(error as NSError):
                return (error.code, error.localizedDescription)
            default:
