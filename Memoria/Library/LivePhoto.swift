...
//  Memoria
//
//  Created by Sagar R Patel on 2021-12-12.
//
...
class LivePhoto {
    ...
    public class func extractResources(from livePhoto: PHAsset, completion: @escaping (LivePhotoResources?) -> Void) {
        queue.async {
    ...
    private static let queue = DispatchQueue(label: "com.memoria.LivePhotoQueue", attributes: .concurrent)
    private lazy var cacheDirectory: URL? = {
        if let cacheDirectoryURL = try? FileManager.default.url(for: .cachesDirectory, in: .userDomainMask, appropriateFor: nil, create: false) {
            let fullDirectory = cacheDirectoryURL.appendingPathComponent("com.memoria.LivePhoto", isDirectory: true)
    ...
    private func extractResources(from livePhoto: PHAsset, to directoryURL: URL, completion: @escaping (LivePhotoResources?) -> Void) {
    ...
    private func extractResources(from livePhoto: PHAsset, completion: @escaping (LivePhotoResources?) -> Void) {
        if let cacheDirectory = cacheDirectory {
