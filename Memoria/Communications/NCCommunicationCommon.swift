...
@objc public class NCCommunicationCommon: NSObject {
    ...
    @objc public let sessionIdentifierDownload: String = "com.memoria.session.download"
    @objc public let sessionIdentifierUpload: String = "com.memoria.session.upload"
    ...
    @objc public let backgroundQueue = DispatchQueue(label: "com.memoria.nccommunication", qos: .background, attributes: .concurrent)
    ...
    @objc public func setup(account: String? = nil, user: String, userId: String, password: String, urlBase: String, userAgent: String, webDav: String?, nextcloudVersion: Int, delegate: NCCommunicationCommonDelegate?) {
        setup(account: account, user: user, userId: userId, password: password, urlBase: urlBase)
        setup(userAgent: userAgent)
        if webDav != nil { setup(webDav: webDav!) }
        setup(nextcloudVersion: nextcloudVersion)
    ...
    @objc public func setup(account: String? = nil, user: String, userId: String, password: String, urlBase: String) {
        if self.account != account {
            NotificationCenter.default.post(name: Notification.Name(rawValue: "changeUser"), object: nil)
        }
    ...
    @objc public func chunkedFile(inputDirectory: String, outputDirectory: String, fileName: String, chunkSizeMB: Int, bufferSize: Int = 1_000_000) -> [String] {
        let fileManager: FileManager = .default
        var isDirectory: ObjCBool = false
        let chunkSize = chunkSizeMB * 1_000_000
